...
- **不代发**：任何回复必须经用户确认（终端确认、卡片点击或通知弹窗「发送」）。
  展示草稿 ≠ 授权发送。
...
- `notify`：P0 系统级通知（macOS 弹窗/横幅）。**零配置默认开启**：文件缺失时
  ...
  不抢焦点，工作零打断）：草稿通知按钮「发送」一键回复、**点横幅正文 =
  复制并跳转**、关闭按钮「忽略」；即时/兜底通知按钮「复制」、点正文 = 跳转；
  VC 通知「加入」或点正文即入会；均 60 秒超时。横幅要有常驻按钮需在
  系统设置 → 通知里把 alerter 的样式设为「提醒」（横幅样式几秒即逝）。
  未装 alerter 时用上述 osascript 弹窗。用户说"来消息
