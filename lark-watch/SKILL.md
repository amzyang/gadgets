...
`link`(applink，点击直达该消息；终端展示须走 OSC 8，见「展示规范」)。
...
「谁在哪发起了会议」+ `link`（OSC 8 输出，标签「加入会议」）让用户点击加入。
系统侧已弹出专用「忽略/加入」
...
5. **展示**：原消息 + 分类 + 草稿 + 洞察，`link` 经 OSC 8 输出成可点击行
   （见「展示规范」）。洞察写有信息量的内容：
...
预览、`link` 直达会话——OSC 8 输出，多会话合并一次 printf）。一两句转述即可；
只有出现值得注意的内容（与用户工作
...
- 转述消息时带上 `link`（`lark://` applink，点击直接唤起飞书客户端定位到
  ...
  的 API，跳转就是等效操作。
- `link` 必须经 OSC 8 超链接输出才可点击：正文里的裸 `lark://` 文本、markdown
  链接、直出的转义序列都不可点（渲染器不为非 http scheme 生成超链接，只有
  Bash stdout 的转义序列原样透传到终端）。转述文本之后紧跟一次 Bash 调用：

  ```
  printf '\033]8;;%s\007👉 %s\033]8;;\007\n' '<link>' '打开「群名/人名」会话'
  ```

  URL 与文案经 `%s` 传参、各自单引号包裹，不要把 URL 内嵌进格式串。标签按
  场景写：会话级 applink「打开会话」、消息级「直达消息」、音视频会议
  「加入会议」。多条链接（补课/digest 按会话一行）在同一次 printf 里追加
  参数对（格式串循环复用），不要发多次 Bash 调用。
...
- **非文本内容先看后判**：细判/起草所依赖的信息在图片、附件、链接文档、
  ...
  可获取的必须获取，不可获取的显式声明「未能查看」。约束跟随细判/起草
  动作本身——补课展开 P0、digest 深入群聊后起草同样适用。
...
2. 输出单行 JSON：`chats[]` 已按「含 P0 的会话优先、条数降序」排好。转述时：
   ...
   「群名（n 条）+ peek 预览 + link（OSC 8，多会话合并一次 printf）」即可；
...
- `context/`：上下文源卡片目录（模型侧配置，二进制不读；改完下一次起草生效）。
  ...
  查 X」时，直接替用户写一张卡进该目录。
- `notify`：P0 系统级通知（macOS 弹窗/横幅）。**零配置默认开启**：文件缺失时
  直接用内嵌于二进制的弹窗，60 秒无操作自动关闭（超时即忽略）、Esc 即忽略。
  草稿联动通知（send-card 后弹出）正文展示对方消息摘要＋候选①全文（看清要发
  什么再按），按钮「忽略/复制并跳转/发送」、回车 = 发送——点「发送」直接以
  候选①回复对方（等价卡片「发送 ①」，幂等防双发，全程无需切回飞书、不打断
  当前工作），点「复制并跳转」把候选①置入剪贴板并进飞书（想手改后发走这条）；
  即时/兜底通知（无草稿）为「忽略/复制/跳转」（无 link 时「复制/OK」）——
  「复制」取消息摘要，「跳转」open applink 唤起飞书定位到消息。
  装有 [alerter](https://github.com/vjeantet/alerter) ≥26
  （`brew install vjeantet/tap/alerter`；≥26 为双横线旗标语法，旧版不兼容）
  时内置通知自动升级为通知中心横幅（每次弹窗现探测 PATH，装完即生效；
  不抢焦点，工作零打断）：草稿通知动作下拉 =「发送」＋常用语＋表情回应
  ...
  弹窗提醒我"时无需任何操作（默认已开启）；说"别弹了/关掉通知"时写入关闭哨兵
  （空文件同义）：
  ...
  模型无需额外操作。联动通知正文带候选①全文：默认弹窗「发送」一键回复
  （回车同）、「复制并跳转」复制话术并进飞书，Esc/超时即忽略（发出后
  pending 失效，卡片按钮再点显示「已失效」，属预期）。
  ...
  要横幅不打断操作，自定义脚本用 `display notification`（走通知中心，
  但横幅点击带不了跳转）。
...
- `notify-vc`：音视频会议（`video_chat`/`vc_meeting`）专用通知命令，覆盖 VC
  批次的弹窗样式；`notify` 仍是通知总开关（notify 为 off 时 notify-vc 不生效）。
  环境变量与 notify 相同，仅 `LW_TITLE` 为「📞 音视频会议」（多条带条数）。
  缺失时用内置弹窗（内嵌于二进制，默认无需配置；装有 alerter 走横幅，
  「加入」或点正文即入会）：osascript 版为「忽略/加入」、默认「加入」，
  点「加入」open 首条 applink 直达会话中的会议消息，60 秒无操作自动关闭。
  响铃与前台抑制同样适用。自定义样式才需要该文件，写自己的脚本
  （argv 传参防注入，同 notify）。
- `quick-replies`：通知横幅的常用语快捷回复，每行一条（`#` 起注释——含 `#`
  的常用语会被截掉，属解析器取舍）。缺失时内置默认「收到」「好的，稍后
  回复」。横幅下拉点选即以该文本回复对应消息（独立幂等键防连点双发，也不
...
- `reactions`：通知横幅的表情回应，每行一个飞书 emoji_type（大写下划线，
  ...
  取 2 个。点选给对应消息加表情回应，不影响草稿候选（点赞 ≠ 已回复）。
  横幅动作总数上限 6（首键＋常用语＋表情），超出截断。
