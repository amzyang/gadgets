...
`resources[]`：daemon 已对消息里的三类常见资源自动预取——正文云文档链接
（docx/wiki）、图片、文件附件。每项 `kind`(doc/image/file)、`name`(文件名/
文档标题)、`ref`(URL 或 file_key)、`mid`(所属消息)、`path`(本地产物路径，
产物在 `~/.local/state/lark-watch/prefetch/`，7 天自动清扫)、`content`
(文档内联正文，超长截断，全文在 `path`)、`err`(预取失败原因)。直接消费：
文档读 `content`（内容明显被截断且细判/起草需要全文时 Read `path`），
图片/文件用 Read 打开 `path`；已预取的资源不要重复拉取。`err` 非空或事件
缺 `resources`（旧二进制/`LW_PREFETCH=0` 关闭）时回退第 2 步手动获取，仍
拿不到按「未能查看」声明；`err` 集中为 auth 类（NeedUserAuthorization）时
提醒用户 `lark-cli auth login --domain im,contact,docs` 补 docs 域授权。
`replied:true` 的事件不预取（只带 ref），本就安静跳过。
...
2. **上下文**（判定需要回应就拉，一次 10 条；纯闲聊/FYI/音视频会议跳过）：
   ...
   非文本内容先看后判（拉取必要性、占位符禁推断等规则以「硬规则」同名条目
   为准，本步只列执行路径）：云文档链接、图片、文件附件三类通常已由 daemon
   预取进事件 `resources[]`（见「P0」字段说明），直接消费即可——本步的
   手动命令仅作回退：`resources` 缺失、某项带 `err`、或内容超出预取三类
   （表格/Base/妙记/名片/合并转发/语音视频，事件 `type` 字段与正文链接
   模式可识别）时才执行。
...
3. **分类**：咨询 / 闲聊 / 任务 / FYI。对方在约会议/约时间（"找个时间对齐"
   ...
   （见下节，与草稿卡并行、互不替代）。消息含**具体未来时间**的会议/截止类
   （"9:20 早会""14:00 前交周报"）另安排「延时提醒」（见同名节，与前两者
   并行）。
4. **草稿**：起草前先注入个人画像（lark-persona 产物；文件存在才用，缺失静默跳过）：
   ...
   代码之一），说不出依据的表态不得单独成稿——只能作为「方向候选」中带方向
   标签的一项出现（立场摆出来、由用户点选拍板，选择权在用户，不算代笔
   立场）。该约束挂在草稿输出上——即使细判没识别出决策类消息，出稿前也要
   过一遍「表态有据吗」；且优先级高于
   ...
   （「可以，未命中直接下一条的话 dwell 确实用不上」）；有据存疑/无据——切
   「方向候选」模式：默认三向，① 立场安全项＋② 肯定＋③ 否定（部署确认：
   缓冲/放行/拦下；方案评审：追问/认可/保留意见），某方向在该消息语义下
   不成立或明显不适用时省去、出 2 条；每条都是该方向下完整可发话术
   （persona 语气），发卡用 `--label` 逐条标注方向、会话内展示同样逐条标注
   （见「卡片确认」）。① 的立场安全项按求证结果取：无据——复述确认式追问
   （把对方理由复述回去请其确认关键前提）或诚实缓冲（「我看下这块再回你」）；
   有据存疑——保留意见＋追问（带上存疑依据）。横幅一键发送只承载①，故 ①
   必须立场安全；用户想手改可点「复制草稿」。
   ...
   候选。候选间必须是不同的应对
   策略，不是同义改写；按推荐度排序，① 放最推荐的。表态门禁的方向候选是
   「多种合理应对策略」的特化：2–3 条、方向对立——立场未经证实时安全项
   （追问/缓冲/保留意见）即最推荐，① 放安全项与本条排序规则一致。
5. **展示**：分类 + 草稿 + 洞察（原消息已由「转述先行」段承担，不重复贴），
   ...
   - 表态门禁场景：写明求证结论——「已核对本会话前文/归档/代码 ✓，依据是…」
     或「未验证对方建议合理性，方向已列，拍板留给你」（方向标注跟着每条
     草稿走、与卡片 `--label` 一致，不写进洞察段）。
   没有洞察就不硬写，只给分类+草稿。
...
方向候选（表态门禁存疑/无据）再加 `--label` 给每条候选标注方向短标签——与
`--draft` 按序配对且等数（数量不符发卡即报错；某位确实不标就给 `--label ''`
空串占位），2–6 个字、不含空格（flag 解析遇空格会静默截断后续参数）；纯展示，
不进正文、不随发送/复制发出。卡片候选标题渲染成「草稿 ①（缓冲）」，手机上
扫标签即选：

```
{SKILL_DIR}/bin/lark-watch send-card --mid <mid> \
  --draft <(printf '%s' '<缓冲话术>') --draft <(printf '%s' '<放行话术>') \
  --draft <(printf '%s' '<拦下话术>') --label '缓冲' --label '放行' --label '拦下' \
  --note '<求证结论>' --original '<原消息文本>' --from '<发送者>' --scene '<私聊|群名>'
```
...
方向候选时如 `--note '未验证上线条件，方向已列，拍板留给你'`；非门禁场景可省略。

用户点「发送」（多候选时「发送 ①/②/③」任一，带方向标签时扫标签选）= 以该
候选发出（单击即发，无二次弹窗，幂等键防连点；发出后其余候选随卡片一并
失效）；「复制草稿」= bot 把全部
候选逐条回发纯文本（长按可复制，手改后自己发）；「忽略」= 丢弃全部候选。点击后
的一切由二进制直接执行，零模型参与。超过 24 小时未处理的草稿由 daemon 自动
过期：卡片改「草稿已过期」并丢弃候选（`LW_PENDING_TTL` 秒数可调，`<=0` 关闭）。
细节与排错见 `{SKILL_DIR}/references/card-confirm-flow.md`。
...
### 延时提醒（含具体未来时间的消息 → 到点横幅）

消息里含**具体未来时间**的会议/截止类内容（"9:20 敬业 早会""14:00 前交
周报"）时，除正常转述外安排一条延时提醒——消息到达时刻的通知会被淹没，
真正需要提醒的是事件临近的时刻：

```
{SKILL_DIR}/bin/lark-watch notify --at '09:18' --mid <源消息mid> \
  --title '⏰ 早会提醒' --message '9:20 敬业 早会（闫旭）' --link <消息applink> \
  --cid <cid> --ctype <ctype> --fid <fid> --from '<发送者名>'
```

- **默认提前 2 分钟**（9:20 的会 → `--at '09:18'`）；跨天写 `MM-DD HH:MM`；
  时间已过会报错——事件已开始/临近就不安排，转述里带一句即可。
- **必带 `--mid`（源消息 id）**：同 mid 覆盖旧提醒，补课/积压重放同一消息
  不会重复弹；改时间 = 同 mid 重发。
- **身份四元组 `--cid/--ctype/--fid/--from` 照抄事件 JSON 同名字段**
  （`--from` 对应 `from`）：到期横幅显示对应头像（p2p=对方、group=群）；
  `--from` 值含空格/引号用单引号包裹（同 `--message`）。人工/无来源提醒
  可整组省略（默认图标）。
- 只对**显式喊话的具体时间**安排；模糊表达（"下午聊""回头对下"）不安排——
  不确定的提醒比不提醒更打扰；周期性日程（每日站会）归用户日历，只有群里
  显式喊话（"9:20 早会"）才安排。
- 命令落盘即退出（不阻塞，无需 `run_in_background`）；到期由 run daemon 弹
  横幅并回报 `reminder` 事件（见同名节）。安排成功后在转述里带一句
  （"已安排 9:18 提醒"）。语法细节与可靠性边界见
  `{SKILL_DIR}/references/notify.md`「手动触发与定时提醒」。
...
**`chats[].resources[]`**：peek（该会话最新一条）消息的三类资源（云文档
链接/图片/文件附件）已随摘要预取，字段与 P0 的 `resources[]` 相同（digest
内联更短、全文在 `path`；仅热度序前 5 个会话、每会话至多 2 项）。预取成功
读 `resources` 即算「已看」，可直接定性与引用内容；某项带 `err` 或字段缺失
时，下述「先看后说」规则照旧兜底。

**peek 是非文本占位符时先看后说**（digest 定性同受「硬规则」先看后判约束）：
`[图片]`/`[文件:名]`/`[卡片:标题]`/`[合并转发]`（旧二进制原文形如
`![Image](…`、`[Image: …`、`<file…`、`<card…`）没有文字线索，定性前必须先
展开实际查看：`+chat-messages-list --chat-id <cid>` 定位该消息拿
mid/msg_type/key，获取方式同「事件处理」第 2 步与
`{SKILL_DIR}/references/content-fetch.md`。看完再定性：确属闲聊/表情包一句
带过即可，与用户工作相关照常提醒；拿不到或无法消费时照实转述
「X 发了图片/文件（未能查看）」，不定性。
...
`{"p":"reminder","title":…,"msg":…,"mid":…,"due":…}`：`notify --at/--in` 安排
的延时提醒到期，横幅已由 daemon 自动弹出（绕开 off 总开关与前台抑制），无需
任何操作。简短转述一句（标题＋内容）即可；`mid` 为安排时的源消息 id（可空），
`due` 为原定提醒时刻。迟到超 15 分钟的提醒（休眠合盖、daemon 中途没在跑）被
自动丢弃、不产生此事件（events.log 记 `reminder.drop`）。
...
- **非文本内容先看后判**：细判/起草所依赖的信息在图片、附件、链接文档、
  卡片等非文本载体里时，必须实际获取查看后才能分类/起草（云文档链接/图片/
  文件附件三类通常已预取进事件 `resources[]`，读之即算「已看」；手动命令见
  「事件处理」的「上下文」步骤，其余类型见
  `{SKILL_DIR}/references/content-fetch.md`）。仅当细判/起草依赖该内容时
  才拉取（对方明说 FYI、`replied` 跳过、纯寒暄不拉），但「无需回应」的
  结论必须能由纯文字部分独立得出——不得看着占位符猜内容再判 FYI。占位符
  与裸链接不是内容，禁止凭周边文字推断（曾发生：把「咨询详情」截图误判为
  Grafana 看板）；
...
- **误报治理**：某类消息反复被推送但用户不关心时，主动建议加噪音规则：
  ...
  拼接串匹配，可压掉 P0；先经正则校验，坏模式会被拒绝；正则以 `-` 开头时
  写成 `ignore-add -- '<regex>'`）。下一 tick 即生效。
...
补课事件不带 `resources`（交互式命令、响应速度优先）：peek 里的非文本内容
与链接文档按「事件处理」第 2 步的回退命令手动获取。
