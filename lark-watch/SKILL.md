...
4. **草稿**：起草前先注入个人画像（lark-persona 产物；文件存在才用，缺失静默跳过）：
   ...
   动作，找不到即止）：① 拉长历史——`--page-size` 提到 50 或线程全量，设计
   讨论的前文（字段用途、schema 由来）常在同一会话；② 归档检索——grep
   `~/.local/share/lark-persona/archive/chats.ndjson` 关键术语（字段名/接口名），
   找此前讨论；③ 本地工作关联——话题命中用户近期工作时，rg 术语定位并阅读
   相关 schema/代码/文档。按求证结果出稿：有据支持——可同意，草稿带一句依据
