...
```
P0 消息 → 模型起草 → lark-watch send-card（pending 入库 + 渲染模板 + bot 发卡给用户本人）
  ├─ 点「发送」→ 二进制：读 pending → +messages-reply --as user → 删 pending → 改卡「✅ 已发送」
  ├─ 点「忽略」→ 二进制：删 pending → 改卡「已忽略」
  └─ 点「复制草稿」→ 二进制：bot 把草稿以纯文本私发给用户（长按/右键即可复制），
...
- 改卡 token 30 分钟/最多 2 次、须整卡替换；用尽时改卡失败仅记 stderr
  （发送本身不受影响）。
- 五分支语义：send（幂等键=mid 防连点双发）/ ignore / copy（不改卡）/
  pending 缺失改卡「已失效」/ reply 失败保留 pending 改卡「发送失败」。
...
| 单元测试 | `cd {SKILL_DIR}/go && go test ./...`（card_test.go 覆盖五分支+去重+模板渲染） |
