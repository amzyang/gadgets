...
- `--note <text>`（表态门禁场景带上）：在全部候选块之后、共享按钮之前追加灰字
  ...
  与引用块一样保留。
- `--label <短标签>`（可重复，方向候选场景）：与 `--draft` 按序配对且等数，
  数量不符发卡即报错（少给会静默错位，多给指向不存在的候选）；全不给 =
  不标注，某位空串 = 该位不标；只渲染进候选块标题括注（「草稿 ①（缓冲）」，
  转义＋换行归一后进加粗标记），不入 pending——发送/复制/横幅都取 pending
  原文，发出的消息天然无标签；完成态只留所选候选块时标签随块保留。
