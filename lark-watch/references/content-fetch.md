...
- **三类常见资源已由 daemon 预取**：正文云文档链接（docx/wiki）、图片、
  文件附件随事件 `resources[]` 带到（P0 事件与 digest peek），直接消费即可、
  不要重复拉取；本表命令是回退路径——`resources` 缺失、某项带 `err`、补课
  事件，或表格/Base/妙记等预取之外的类型时才用。
- 拉取必要性与占位符禁推断（`[File: xxx]`、裸链接、卡片 JSON 都不是内容
  本身），以 SKILL.md「硬规则」的「非文本内容先看后判」条目为准；本表只管
  「怎么拿」。
