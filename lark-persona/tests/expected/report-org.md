# lark-persona 关系图谱草表

纯统计推断（本地归档，零 LLM）。请人工校正「上下级推断」后落 `seeds.yaml`。

## Top 20 高频伙伴

| # | 姓名 | 总量 | p2p 我/TA | 发起 我/TA | 回复中位min 我/TA | 短确认 我/TA | 指派 我/TA | @ 我→TA/TA→我 | 共同群 | 推断 |
|---|------|------|-----------|------------|--------------------|---------------|-------------|----------------|--------|------|
| 1 | 老板 | 10 | 5/5 | 0/1 | 1/44 | 5/0 | 0/5 | 0/0 | 0 | 直属上级 |
| 2 | Alice | 6 | 3/3 | 1/1 | 2/3 | 1/2 | 0/1 | 0/0 | 0 | 同组平级 |
| 3 | Bob | 2 | - | - | - | - | - | 1/1 | 1 | 平级/待定 |
| 4 | Carol | 1 | - | - | - | - | - | 0/0 | 1 | 直属下属 |

## 上下级推断草表（含证据，请校正）

- **老板** → 直属上级（通讯录 ground truth）
- **Alice** → 同组平级（通讯录 ground truth）
- **Carol** → 直属下属（通讯录 ground truth）

## seeds.yaml 模板（校正后存 ~/.local/share/lark-persona/seeds.yaml）

```yaml
# 人工覆盖层（优先级最高，高于通讯录 org.ndjson 与统计推断；只写需纠正的人）
leader:            # 直属上级 open_id 或姓名
peers: []          # 平级协作
reports: []        # 下属
```
